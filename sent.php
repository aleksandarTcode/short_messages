...
<table class='table table-bordered mt-5'>
    ...
    <?php
        ...
        <tr>
            ...
            <?php $row = $user->messages_for_current_user_with_pagination('SELECT id,recipient,msg_text,time,read_msg FROM msg WHERE sender = ? ORDER BY time DESC', $user->username);
            if($row){
            foreach($row as $msg){
            ?>
        <tr>
