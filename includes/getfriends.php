    ...
    foreach($result_array as $friend) {
        ...
        if (strpos($friends_search,$q)!==false) {
            ...
            if ($hint === "" && $user->username!==$friend->username && $user->check_if_user_and_friend_in_search_are_already_friends($friend->username)==false) {
               $hint = <<<DEILMITER
  <div style="min-height: 65px"><br><a href="add_friend_from_search.php?friend_username={$friend->username}" class="btn btn-outline-info btn-sm ml-2">Add <strong>{$friend->username}</strong> as a friend</a><img src='img/profile_photos/{$friend->user_photo}' style='float: right; width: 60px; height: 60px; object-fit: cover;'></div>
DEILMITER;
